...
class AccrualService:
    ...
    def record_kills(
        self, user: User, kills: int, epoch_minute: int | None = None
    ) -> AccrualResult:
        if kills <= 0:
            return AccrualResult(
                user_id=user.id, kills=0, amount_ban=0.0, epoch_minute=epoch_minute or 0
            )
        ...
        accrual = RewardAccrual(
            user_id=user.id, kills=kills, amount_ban=amount, epoch_minute=minute
        )
