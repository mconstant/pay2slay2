...
def test_oauth_state_single_use(client):
    ...
    # First call succeeds (302 redirect)
    first = client.get(f"/auth/discord/callback?state={state}&code=dummy", follow_redirects=False)
    assert first.status_code == HTTPStatus.FOUND, first.text
    # Replay attempt with same state now rejected
    second = client.get(f"/auth/discord/callback?state={state}&code=dummy", follow_redirects=False)
...
def test_oauth_state_mismatch(client):
    ...
    resp = client.get(
        "/auth/discord/callback?state=not_a_valid_state&code=dummy", follow_redirects=False
    )
...
def test_oauth_state_tampered_signature(client):
    ...
    resp = client.get(f"/auth/discord/callback?state={flipped}&code=dummy", follow_redirects=False)
    assert resp.status_code == HTTPStatus.BAD_REQUEST
